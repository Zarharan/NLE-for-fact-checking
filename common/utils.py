...
class Summarization():
    ...
    def get_summary(self, text_for_summary, claim_id, claim):
        ...
        less_than_max_token= True if (len(main_text_words) * (4/3))<= (self.max_tokens - 50) else False

        model_name_for_save= (self.model_name+"_not_summarized_" + str(self.max_tokens)) if less_than_max_token else self.model_name

        # Read and return summary from database if for the target claim, the summary was already generated by the target model
        select_result= self.text_summary.select_summary(claim_id, model_name_for_save)
        if select_result:
            return select_result.summary

        # Return the main text if its token count is less than max tokens. 50 is for tolerance
        if less_than_max_token:
            # Save summary into the related table for later
            summary_data= SummaryModel(claim_id= claim_id, main_text= text_for_summary
                , summary= text_for_summary, model_name= model_name_for_save)

            self.text_summary.insert(summary_data)
            self.not_summarized_count += 1
        ...
        # Save summary into the related table for later
        summary_data= SummaryModel(claim_id= claim_id, main_text= text_for_summary
            , summary= summary, model_name= model_name_for_save)
        self.text_summary.insert(summary_data)        
        return summary
    ...
    def __chat_gpt(self, text_for_summary, claim, model="gpt-3.5-turbo"):
        ...
        Extract all important information from the article below, focusing on any aspects that are relevant to the claim below. Limit to {} words.
        claim: ```{}```
        article: ```{}```"""
